...
class SecurityHelper( object ):
    ...
    def __init__( self, **config ):
        ...
        class_id_secret = config.get( 'per_kind_id_secret_base', self.id_secret )
        self.class_id_ciphers = _cipher_cache( class_id_secret )
    ...
    def __id_cipher( self, kind ):
        if not kind:
            ...
            id_cipher = self.class_id_ciphers[ kind ]
        return id_cipher


class _cipher_cache( collections.defaultdict ):

    def __init__( self, secret_prefix ):
        self.secret_prefix = secret_prefix

    def __missing__( self, key ):
        return Blowfish.new( self.secret_prefix + "__" + key )
