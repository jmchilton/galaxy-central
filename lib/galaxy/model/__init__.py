...
class WorkRequest( object, Dictifiable ):
    dict_collection_visible_keys = [ 'id', 'name', 'type', 'state' ]
    dict_element_visible_keys = [ 'id', 'name', 'type', 'state' ]
    states = Bunch(
        ACTIVE='active',  # The request is still valid - either something is working or needs to.
        COMPLETE='complete',  # Request is complete - maybe finished fine, maybe errored,
                              # maybe cancelled - can add more informative state options later
                              # but for now what is actionable is if the work needs to be scheduled.
    )

    def __init__( self, state=states.ACTIVE, uuid=None ):
        if uuid is None:
            self.uuid = uuid4()
        else:
            self.uuid = UUID(str(uuid))
    ...
    def to_dict( self, view='collection', value_mapper=None ):
        rval = super( WorkRequest, self ).to_dict( view=view, value_mapper=value_mapper )
        rval['uuid'] = str( self.uuid )
        return rval


class WorkflowRequest( WorkRequest ):
...
class WorkRequestReserve(object, Dictifiable):
    dict_collection_visible_keys = [ "id", "destination_id", "state", "update_time" ]
    dict_element_visible_keys = [ "id", "destination_id", "state", "update_time" ]
