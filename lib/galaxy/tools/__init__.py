...
from galaxy.tools.lineages import LineageMap
...
class ToolBox( object, Dictifiable ):
    ...
    def __init__( self, config_filenames, tool_root_dir, app ):
        ...
        self.tool_panel = ToolPanelElements()
        self.index = 0
        ...
        self.integrated_tool_panel = ToolPanelElements()
    ...
    def __add_tool_to_tool_panel( self, tool, panel_component, section=False ):
        ...
        if already_loaded:
            ...
            inserted = False
            index = self.integrated_tool_panel.index_of_tool_id( tool_id )
            if index:
            ...
            if not inserted:
                ...
                for lineage_id in tool.lineage_ids:
                    index = self.integrated_tool_panel.index_of_tool_id( lineage_id )
                    if index:
                        panel_dict.insert_tool( index, tool )
                        inserted = True
                if not inserted:
    ...
    def get_tool_id( self, tool_id ):
        ...
        tools (using get_tool for inexact matches).
        """
    ...
    def load_section_tag_set( self, elem, tool_path, load_panel_dict, index=None ):
        ...
        self.integrated_tool_panel.update_or_append( index, key, integrated_section )
    ...
    def load_hidden_tool( self, config_file ):
...
class Tool( object, Dictifiable ):
    ...
    def tool_version_ids( self ):
        ...
        tool_version = self.tool_version
        if tool_version:
            return tool_version.get_version_ids( )
        return []
