...
from .resolver_mixins import UsesHomebrewMixin, UsesToolDependencyDirMixin
from ..resolvers import DependencyResolver, INDETERMINATE_DEPENDENCY

log = logging.getLogger(__name__)


class HomebrewToolShedDependencyResolver(DependencyResolver, UsesHomebrewMixin, UsesToolDependencyDirMixin):
    ...
    def _find_tool_dependencies(self, name, version, type, **kwds):
        ...
        if installed_tool_dependency:
            return self._resolve_from_installed_tool_dependency( installed_tool_dependency )
        ...
        if "tool_dir" in kwds:
            ...
            if os.path.exists(tool_depenedencies_path):
                return self._resolve_from_tool_dependencies_path(tool_depenedencies_path)
    ...
    def _resolve_from_installed_tool_dependency(self, name, version, installed_tool_dependency):
        ...
        return self._find_dep_default( recipe_name )
