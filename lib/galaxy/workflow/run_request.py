...
def workflow_run_config_to_request( trans, run_config, workflow ):
    ...
    for step in workflow.steps:
        state = step.state
        step_type = step.type
        if step_type in [ None, "tool" ]:
            tool = getattr( step.module, "tool", None )
            serialized_state = state.encode( tool, trans.app )
        else:
            serialized_state = state.inputs
        step_state = model.WorkflowRequestStepState()
        step_state.workflow_step_id = step.id
        step_state.value = serialized_state
