...
class WorkflowRequestMonitor( object ):
    ...
    def __attempt_schedule( self, workflow_scheduler_id, workflow_scheduler ):
        work_queue = self.app.work_queue
        workflow_request = None
        try:
            log.info("GETTING RESERVE")
            reserve = work_queue.get_reserve( model.WorkflowRequest, workflow_scheduler_id )
            log.info("GOT RESERVE %s" % reserve)
            workflow_request = reserve and reserve.request
        except Exception:
            log.exception( "Exception raised while attempting to reserve workflow request." )
            return False
        ...
        try:
            workflow_scheduler.schedule( workflow_request )
        except Exception:
            log.exception( "Exception raised while attempting to schedule workflow request." )
            return False
        try:
            work_queue.complete_request( workflow_request )
        except Exception:
            log.exception( "Exception raised while attempting to complete workflow request." )
