...
class LwrExchange(object):
    ...
    def __init__(
        ...
        self.__publish_kwds = publish_kwds.copy()
    ...
    def connection(self, connection_string, **kwargs):
        ...
        if "heartbeat" not in kwargs:
            kwargs[ "heartbeat" ] = self.__heartbeat
            # If heartbeat > 0, do we need launch thread to
            # produce beats (call heartbeat_check) or does
            # library do this for us?
