...
from galaxy.jobs.runners.util.condor import build_submit_description, condor_submit, condor_stop, summarize_condor_log
...
class CondorJobRunner( AsynchronousJobRunner ):
    ...
    def queue_job( self, job_wrapper ):
        ...
        cjs.exit_code_file = os.path.join( self.app.config.cluster_files_directory, 'galaxy_%s.ec' % galaxy_id_tag )
        ...
        build_submit_params = dict(
            ...
            query_params=job_destination.params,
        )
        submit_description = build_submit_description( **build_submit_params )
        job_script_params = {
        ...
        script = job_script(**job_script_params)
        try:
    ...
    def check_watched_items( self ):
        ...
        for cjs in self.watched:
            ...
            try:
                ...
                job_complete = s5
                job_failed = s9
                job_running = s1 and not s4 and not s7
    ...
    def stop_job( self, job ):
        ...
        failure_message = condor_stop( job.job_runner_external_id )
        if not failure_message:
            log.debug( "(%s/%s) Removed from DRM queue at user's request" % ( job.id, job.job_runner_external_id ) )
        else:
            log.debug( "(%s/%s) User killed running job, but %s" % ( job.id, job.job_runner_external_id, failure_message ) )
