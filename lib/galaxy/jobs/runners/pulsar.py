...
class PulsarLegacyJobRunner( AsynchronousJobRunner ):
    """
    Pulsar Job Runner
    """
    runner_name = "PulsarRunner"

    def __init__( self, app, nworkers, **kwds ):
        """Start the job runner """
        super( PulsarLegacyJobRunner, self ).__init__( app, nworkers, runner_param_specs=PULSAR_PARAM_SPECS, **kwds )
    ...
    def check_watched_item(self, job_state):
        try:
            ...
            return None
        job_state = self.__update_job_state_for_status(job_state, status)
        return job_state

    def __update_job_state_for_status(self, job_state, pulsar_status):
    ...
    def __async_update( self, full_status ):
        ...
        try:
            ...
            self.__update_job_state_for_status(job_state, full_status["status"])
        except Exception:
    ...
    def queue_job(self, job_wrapper):
        ...
        try:
            dependencies_description = PulsarLegacyJobRunner.__dependencies_description( client, job_wrapper )
            rewrite_paths = not PulsarLegacyJobRunner.__rewrite_parameters( client )
    ...
    def __prepare_job(self, job_wrapper, job_destination):
        ...
        try:
            ...
            rewrite_parameters = PulsarLegacyJobRunner.__rewrite_parameters( client )
            prepare_kwds = {}
            ...
            remote_metadata = PulsarLegacyJobRunner.__remote_metadata( client )
            dependency_resolution = PulsarLegacyJobRunner.__dependency_resolution( client )
    ...
    def finish_job( self, job_state ):
        ...
        try:
            ...
            return
        if not PulsarLegacyJobRunner.__remote_metadata( client ):
    ...
    def shutdown( self ):
        super( PulsarLegacyJobRunner, self ).shutdown()
    ...
    def __dependencies_description( pulsar_client, job_wrapper ):
        dependency_resolution = PulsarLegacyJobRunner.__dependency_resolution( pulsar_client )
    ...
    def __build_metadata_configuration(self, client, job_wrapper, remote_metadata, remote_job_config):
        ...
        if remote_metadata:
            ...
            if PulsarLegacyJobRunner.__use_remote_datatypes_conf( client ):
