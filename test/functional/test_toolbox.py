...
class ToolTestCase( TwillTestCase ):
    ...
    def __galaxy_interactor( self, testdef ):
        force_api_interactor = string_as_bool( os.environ.get( "GALAXY_TEST_FORCE_API_INTERACTOR", "false" ) )
        use_api_interactor = force_api_interactor
        if use_api_interactor:
            galaxy_interactor = GalaxyInteractorApi( self )
        else:
...
class GalaxyInteractorApi( object ):
    ...
    def verify_output( self, history_id, output_data, outfile, attributes, shed_tool_id, maxseconds ):
        ...
        try:
            data = self.__get( "histories/%s/contents/%s/display" % (history_id, hid) ).text
            self.twill_test_case.verify_hid( outfile, data=data, hda_id=hid, attributes=attributes, shed_tool_id=shed_tool_id )
        except Exception:
            ## TODO: Print this!
    ...
    def stage_data_async( self, test_data, history_id, shed_tool_id, async=True ):
        ...
        tool_input = {
            ...
            "dbkey": "?",  # TODO: Handle it! Doesn't work if undefined, does seem to in Twill.
            "files_0|NAME": name,
