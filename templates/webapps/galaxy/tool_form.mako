    ...
    $(function() {
        ...
        var enableSwitchOption = function(optionElement, onValue) {
            $(optionElement).children(":input").each(function(i, formElement) {
                ...
                var disabledSuffix = "|__disabled__|" + onValue;
                var newName = name.substring(0, name.length - disabledSuffix.length);
                if(name.endsWith(disabledSuffix)) {
        ...
        var disableSwitchOption = function(optionElement, onValue) {
            $(optionElement).children(":input").each(function(i, formElement) {
                ...
                var disabledSuffix = "|__disabled__|" + onValue;
                if(! name.endsWith(disabledSuffix)) {
                    $(formElement).attr('name', name + disabledSuffix);
                }
            });
            $(optionElement).hide();
        };
